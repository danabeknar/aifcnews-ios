...
class FeedViewController: UIViewController {
    ...
    private var newsTitles = Set<String>()
    private lazy var reachability = Reachability()!
    
    private lazy var menuView: BTNavigationDropdownMenu = {
        let menuView = BTNavigationDropdownMenu(navigationController: self.navigationController,
                                                containerView: (self.navigationController?.view!)!,
                                                title: BTTitle.title("Menu"), items: [])
    ...
    private func updateMenuView(with tags: [Tag]) {
        var items = [String]()
        for tag in tags {
    ...
    private func parseData() {
        NotificationCenter.default.addObserver(self, selector: #selector(checkConnectivity),
    ...
    func fetchData(with tag: Tag) {
        News.fetchNews(with: tag) { (data, _) in
            if let data = data {
                self.news = data
            }
